...
By defualt a `xts` time series object. To make life easier we will convert this to a tibble (a data frame like object following the tidy data  philosophy). Tibbles are nearly identical to data frames, but have much nicer print methods. The variable names of `tab` are annoying always being prefixed by `DATA`, so we'll remove this prefix.
...
The final data processing step before we begin is turning price into returns. When analysing risk, we look at Profit/Loss (P/L) figures, not price. Some people prefer to think of Loss/Profit (L/P) instead because we are focusing on the potential **losses** not potential returns, but this is just a preference. 

> To make things very clear, mmr *always* expects P/L data.

One is just the negative of the other: $P/L = -L/P$
